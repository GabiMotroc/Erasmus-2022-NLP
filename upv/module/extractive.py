...
class ExtractiveSummarizer(pl.LightningModule):
    ...
    def __init__(self, hparams, embedding_model_config=None, classifier_obj=None):
        ...
        hparams.tokenizer_no_use_fast = getattr(
            hparams, "tokenizer_no_use_fast", False)
        ...
        self.word_embedding_model = AutoModel.from_config(
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.forward_modify_inputs_callback:
            inputs = self.forward_modify_inputs_callback(
                inputs)  # skipcq: PYL-E1102
    ...
    def prepare_data(self):
        ...
        def get_inferred_data_type(dataset_files):
            dataset_files_extensions = [
                os.path.splitext(x)[1] for x in dataset_files]
            dataset_files_extensions_equal = len(
                set(dataset_files_extensions)) <= 1
        ...
        for corpus_type in data_splits:
            ...
            path = self.hparams.data_path, "*" + corpus_type + ".*"
            ...
            if (not dataset_files) or (self.hparams.only_preprocess):
                json_files = glob.glob(
                    os.path.join(self.hparams.data_path,
                                 "*" + corpus_type + ".json")
                )
                if len(json_files) == 0:
                    logger.error(
    ...
    def validation_epoch_end(self, outputs):
        ...
        avg_loss_total = torch.stack(
            [x["val_loss_total"] for x in outputs]).mean()
        ...
        avg_val_acc_and_f1 = torch.stack(
            [x["val_acc_and_f1"] for x in outputs]).mean()
        ...
        self.log("val_loss", loss_dict["val_" +
                 self.hparams.loss_key], sync_dist=True)
    ...
    def test_step(self, batch, batch_idx):
        ...
        sorted_ids = (
            torch.argsort(outputs, dim=1,
                          descending=True).detach().cpu().numpy()
        )
        if self.hparams.test_id_method == "top_k":
            ...
            indexes = np.argwhere(
                outputs.detach().cpu().numpy() > self.hparams.test_k)
    ...
    def predict_sentences(
        ...
        if tokenized:
            src_txt = [
                " ".join(
                    [token.text for token in sentence if str(token) != "."]) + "."
            ...
            src_txt = [
                " ".join([token.text for token in nlp(
                    sentence) if str(token) != "."])
                + "."
                for sentence in input_sentences
            ]
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument(
            "--classifier",
            type=str,
            choices=["linear", "simple_linear",
                "transformer", "transformer_linear"],
