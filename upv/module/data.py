        ...
        if key == "input_ids":
            ...
            if "sent_lengths" in elem:
                ...
                sent_lengths_width = max(
                    len(d["sent_lengths"]) + 1 for d in batch)
                for d in batch:
                    ...
                    total_value_to_add = input_ids_width - \
                        sum(current_sent_lens)
                    while num_to_add > 1:
                        num_to_add -= 1
                ...
                final_dictionary["sent_lengths_mask"] = torch.tensor(
                    sent_lengths_mask)
...
class SentencesProcessor:
    ...
    def get_features_process(
        ...
        if ex_index % 1000 == 0:
            logger.info("Generating features for example %s/%s",
                        ex_index, num_examples)
        if bert_compatible_cls:
        ...
        if ex_index < 5 and self.verbose:
            ...
            if segment_ids is not None:
                logger.info(
                    "token_type_ids: %s", " ".join(
                        [str(x) for x in segment_ids])
                )
            if sent_rep_ids is not None:
                logger.info(
                    "sent_rep_token_ids: %s", " ".join(
                        [str(x) for x in sent_rep_ids])
                )
            if sent_lengths is not None:
                logger.info(
                    "sent_lengths: %s", " ".join(
                        [str(x) for x in sent_lengths])
                )
            if create_attention_mask:
                logger.info(
                    "attention_mask: %s", " ".join(
    ...
    def get_features(
        ...
        if save_to_path:
            final_save_name = save_to_name if save_to_name else (
                "dataset_" + self.name)
